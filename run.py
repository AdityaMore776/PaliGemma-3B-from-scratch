...
local_download_path = "./paligemma-3b-mix-224-local"
os.makedirs(local_download_path, exist_ok=True)

if not os.path.exists(os.path.join(local_download_path, "config.json")):
  downloaded_model_path = snapshot_download(repo_id=model_id, local_dir=local_download_path)
else:
  downloaded_model_path = local_download_path
...
def get_model_inputs(processor: PaliGemmaProcessor, prompt: str, image_file_path: str, device: str):
  ...
  image = [image]
  prompts = [prompt]
  model_inputs = processor(text=prompts, images=image)
